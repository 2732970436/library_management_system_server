...
import com.zx.server.exception.SystemException;
...
@RestController
@RequestMapping("/salary/api/profile")
...
//      检查用户名和密码是否正确
  ...
  public Result uploadAvatar(@PathVariable Long userId, @RequestParam("file") MultipartFile file) {
    try {
      ...
      File avatar = new File(avatarDir, userId.toString());
  ...
  public Result updateProfile(@RequestBody User user) {
    if (session.getAttribute("userId") != user.getId().toString() && !env.equals("development")) {
